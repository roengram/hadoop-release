...
public class TestRegisterNodeManagerRequest {
  ...
  public void testRegisterNodeManagerRequest() {
    RegisterNodeManagerRequest request =
        RegisterNodeManagerRequest.newInstance(
            ...
            "version", Arrays.asList(ContainerStatus.newInstance(
                ContainerId.newInstance(
                    ApplicationAttemptId.newInstance(
                        ApplicationId.newInstance(1234L, 1), 1), 1),
                ContainerState.RUNNING, "good",
    ...
    // check values
    Assert.assertEquals(request1.getContainerStatuses().size(), request
        .getContainerStatuses().size());
    Assert.assertEquals(request1.getContainerStatuses().get(0).getContainerId(),
  ...
  public void testRegisterNodeManagerRequestWithNullArrays() {
    ...
    // check values
    Assert.assertEquals(0, request1.getContainerStatuses().size());
