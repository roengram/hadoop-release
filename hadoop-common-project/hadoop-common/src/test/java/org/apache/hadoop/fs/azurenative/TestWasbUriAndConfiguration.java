...
public class TestWasbUriAndConfiguration {
  ...
  public void testConnectToFullyQualifiedAccountMock() throws Exception {
    ...
    fs.initialize(new URI(
        "wasb://mockContainer@mockAccount.mock.authority.net"),
        conf);
  ...
  public void testConnectToRoot() throws Exception {
    ...
    final String blobPrefix =
        String.format ("wasbtests-%s-%tQ-blob", System.getProperty("user.name"), new Date());
  ...
  public void testNoUriAuthority() throws Exception {
    ...
    for (String defaultScheme : wasbAliases)
      for (String wantedScheme : wasbAliases) {
        ...
        URI defaultUri =
            new URI(defaultScheme, authority, null, null, null);
        conf.set("fs.default.name", defaultUri.toString());
        URI wantedUri =
            new URI(wantedScheme + ":///random/path");
        NativeAzureFileSystem obtained =
            (NativeAzureFileSystem)FileSystem.get(wantedUri, conf);
        assertNotNull(obtained);
        assertEquals(
            new URI(wantedScheme, authority, null, null, null),
            obtained.getUri());
        // Make sure makeQualified works as expected
        Path qualified = obtained.makeQualified(new Path(wantedUri));
        assertEquals(
            new URI(wantedScheme, authority, wantedUri.getPath(), null, null),
            qualified.toUri());
