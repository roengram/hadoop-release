 ...
 */
public class SnapshotDiffReport implements Writable {
  static {                                      // register a ctor
    WritableFactories.setFactory(SnapshotDiffReport.class,
        new WritableFactory() {
   ...
   * have happened.
   */
  public static class DiffReportEntry implements Writable {
    static {                                      // register a ctor
      WritableFactories.setFactory
        (DiffReportEntry.class,
         new WritableFactory() {
           public Writable newInstance() { return new DiffReportEntry(); }
         });
    }
    ...
    public DiffReportEntry() {
      relativePath = DFSUtil.EMPTY_BYTES; 
    }
    ...
    public void write(DataOutput out) throws IOException {
      Text.writeString(out, type.label);
      out.writeShort(relativePath.length);
      out.write(relativePath);
    }

    @Override
    public void readFields(DataInput in) throws IOException {
      type = DiffType.getTypeFromLabel(Text.readString(in));
      short length = in.readShort();
      relativePath = length == 0 ? DFSUtil.EMPTY_BYTES : new byte[length];
      if (length > 0) {
        in.readFully(relativePath);
      }
    }
  }
  ...
  @Override
  public void write(DataOutput out) throws IOException {
    Text.writeString(out, snapshotRoot);
    Text.writeString(out, fromSnapshot);
    Text.writeString(out, toSnapshot);
    out.writeInt(diffList.size());
    for (DiffReportEntry entry : diffList) {
      entry.write(out);
    }
  }
  ...
  @Override
  public void readFields(DataInput in) throws IOException {
    snapshotRoot = Text.readString(in);
    fromSnapshot = Text.readString(in);
    toSnapshot = Text.readString(in);
    int num = in.readInt();
    for (int i = 0; i < num; i++) {
