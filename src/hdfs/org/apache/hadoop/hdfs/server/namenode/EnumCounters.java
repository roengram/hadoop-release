...
public class EnumCounters<E extends Enum<E>> {
  ...
  public EnumCounters(final E[] enumConstants) {
    for(int i = 0; i < enumConstants.length; i++) {
      if (enumConstants[i].ordinal() != i) {
        throw new IllegalArgumentException("The ordinal of the enum "
            + enumConstants[i].name() + " is " + enumConstants[i].ordinal()
            + ", but the expected value is " + i);
      }
    }
